...
class SRDataset(Dataset):
    ...
    def get_names(folder_list):
        ...
        for folder_ in folder_list:
            folder_ = os.path.expanduser(folder_)  ## to compatible with folder in format of "~/xxx"
    ...
    def load_HR(self):
        if self.pool_hr:
            print('Loading od_images parallelly.')
            ...
            print('Loading od_images sequentially.')
            for i in tqdm(range(len(self.hr_names)), desc="Preloading HR images", total=len(self.hr_names), leave= False):
    ...
    def load_LR(self):
        if self.pool_lr:
            ...
            for i, val in tqdm(enumerate(pool.imap(imread, self.lr_names), 0),
                               ...
                               total=len(self.lr_names), leave = False):
                self.LR_images[i] = val
            pool.close()
            pool.join()
        else:
            for i in tqdm(range(len(self.lr_names)), desc="Preloading LR images", total=len(self.lr_names), leave = False):
